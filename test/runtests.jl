...
@testitem "cds self-contained" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/cds.dat"))
...
@testitem "cds description" begin
    ...
    t = AstroTables.read(data; readme)
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 2
end
...
@testitem "cds multi header" begin
    ...
    t1 = AstroTables.read(joinpath(@__DIR__, "data/cds/multi/lhs2065.dat"); readme)
    ...
    t2 = AstroTables.read(joinpath(@__DIR__, "data/cds/multi/lp944-20.dat"); readme)
...
@testitem "cds glob header" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/cds/glob/lmxbrefs.dat"); readme)
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 291
...
@testitem "vizier table1" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/vizier/table1.dat"); readme)
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 15
...
@testitem "cds null" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/cds/null/table.dat"); readme)
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 2
    ...
    t1 = AstroTables.read(joinpath(@__DIR__, "data/cds/null/table1.dat"); readme=readme1)
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t1, n)))), propertynames(t1))
    @test length(t1) == 2
...
@testitem "mrt dashes" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/cds_mrt_dashes.txt"))
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 8
...
@testitem "mrt large" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/cds2.dat"))
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 215
...
@testitem "cds no data" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/no_data_cds.dat"))
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 0
...
@testitem "cds functional" begin
    ...
    t = AstroTables.read(joinpath(@__DIR__, "data/cdsFunctional.dat"))
    @test all(n -> isconcretetype(nonmissingtype(eltype(getproperty(t, n)))), propertynames(t))
    @test length(t) == 1
...
@testitem "units" begin
    ...
    t_raw = AstroTables.read(joinpath(@__DIR__, "data/cds.dat"); units=false)
    ...
    readme = joinpath(@__DIR__, "data/cds/null/ReadMe")
    tn = AstroTables.read(joinpath(@__DIR__, "data/cds/null/table.dat"); readme)
    ...
    tf = AstroTables.read(joinpath(@__DIR__, "data/cdsFunctional.dat"))
